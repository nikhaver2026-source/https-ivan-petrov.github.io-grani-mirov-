/* ════════════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════════════ */
/* ── ВЕРСИЯ ОБОЛОЧКИ И ВЕРСИЯ ЗАПИСЕЙ РАЗВЕДЕНЫ ──
   Страница берётся «сначала сеть, потом кэш», поэтому обновление доходит до
   игрока с сетью сразу. Но у того, кто зашёл без сети, в кэше лежала бы
   старая игра — и лежала бы там вечно, пока имя кэша не изменится. Поэтому
   имя кэша СТРАНИЦЫ поднимается с каждым обновлением: на первом же заходе
   старая оболочка удаляется целиком.
   Кэш ЗАПИСЕЙ при этом не трогаем нарочно, и имя у него прежнее. Записи
   неизменяемы, их в банке 459 мегабайт, и перекачивать их заново ради новой
   страницы — значит наказать игрока за обновление. */
const SHELL="grani-shell-v13";
