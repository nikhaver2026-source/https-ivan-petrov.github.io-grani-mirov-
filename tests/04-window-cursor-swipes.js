...
(async()=>{
 ...
 /* Ощупывание кончилось миг назад, и его отрыв мог взвести двойное касание:
    прилипшая цель держится 800 мс (tapInLayer). Пауза здесь должна быть
    ДЛИННЕЕ этого окна, иначе проверка меряет не одиночное касание, а второе
    касание двойного — и оно, как ему и положено, выполняет пункт и сбрасывает
    выбор. Четырёхсот миллисекунд не хватало: под нагрузкой это выходило то
    так, то эдак. */
