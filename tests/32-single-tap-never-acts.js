...
(async()=>{
 ...
 for(const [имя,open] of WINDOWS){
  ...
  for(let i=0;i<сколько;i++){
   ...
   /* Одиночное касание молчит: имя пункта оно не произносит — называют
      свайп и ощупывание. Если пункт всё же назвался, это нарушение. */
   else if(r.сказано.some(t=>t.replace(/\s+/g," ").trim().startsWith(т.ждём.slice(0,20))))
    немые.push({окно:имя,пункт:т.что,ждали:"тишина",сказано:r.сказано.slice(0,2)});
  }
 }
 ...
 check('ни одно одиночное касание ничего не выполняет',
  !нарушения.some(x=>x.открылось===undefined),нарушения.filter(x=>x.открылось===undefined));
 check('ни одно одиночное касание не называет пункт: оно молчит',
 ...
 check('в каталоге звуков одно касание не включает звук и молчит',
  послеЭнц.играет===энц.играло&&!послеЭнц.акт.length&&послеЭнц.сказано===0,
 ...
 const полеR=await page.evaluate(()=>({акт:window.__acts.slice(),кмд:window.__cmds.slice(),
  сказано:window.__said.slice()}));
 check('на игровом поле одно касание ничего не делает и молчит',
