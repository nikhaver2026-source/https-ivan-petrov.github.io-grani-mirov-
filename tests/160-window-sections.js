/* ═══════════════════════════════════════════════════════════════════════
   ...
   9. Настройки разбиты на двенадцать разделов, и ни одна не пропала.
...
(async()=>{
 ...
 const настройки=await page.evaluate(()=>{
  ...
  return r;});
 check('настройки разбиты на двенадцать разделов с разными именами, и у каждого своя подпись',
  настройки.разделов===12&&new Set(настройки.имена).size===12
  &&настройки.подписи===12&&настройки.имена.includes("Синтезатор речи"),настройки);
 check('ни одна настройка не осталась вне разделов, и сверху есть оглавление с перескоком',
  /* Кнопки сохранения стоят над разделами нарочно: до них должно быть
     рукой подать из любого места окна, а не после двенадцати заголовков. */
  настройки.вне.every(x=>["close","secindex","secjump","secback","setsave","setsaved"].includes(x))
