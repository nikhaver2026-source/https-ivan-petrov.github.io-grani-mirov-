...
(async()=>{
 ...
 check('в начале боя окно НЕ открывается', await page.evaluate(()=>(document.getElementById('lootOverlay')||{hidden:true}).hidden===true&&activeLayer()===null));
 ...
 await page.evaluate(()=>{G.inCombat=false;G.combat=null;var _b=document.getElementById('combatBar');if(_b)_b.hidden=true;window.__calls=0;});
