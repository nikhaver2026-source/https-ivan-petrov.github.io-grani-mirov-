...
(async()=>{
 ...
 const реестр=await page.evaluate(()=>{
  ...
  /* Каждому умению Грани кто-нибудь да учит. Тёмные умения — исключение
     нарочное: им не учат нигде по эту сторону, их дают дела и обряды
     тёмных земель, и это проверяется отдельно, ниже. */
  const дают=new Set();
  SCHOOL_DB.forEach(sc=>sc.ступени.forEach(ст=>{if(ст.даёт.умение)дают.add(ст.даёт.умение);}));
  const тёмные=new Set(typeof DARK_ABILITY_IDS!=="undefined"?DARK_ABILITY_IDS:[]);
  const сироты=ABILITIES.filter(a=>!дают.has(a.id)&&!тёмные.has(a.id)).map(a=>a.id);
  /* А тёмные умения должны доставаться из тёмных земель — и только оттуда. */
  const изТьмы=new Set();
  for(let x=0;x<300;x+=3)for(let y=0;y<300;y+=7){
   const d=darkDeedAt(x,y);
   if(d.награда.вид==="умение")изТьмы.add(d.награда.что);}
  const тёмныеБезИсточника=[...тёмные].filter(id=>!изТьмы.has(id));
  const тёмныеВШколах=[...тёмные].filter(id=>дают.has(id));
  return {школ:SCHOOL_DB.length,умений:ABILITIES.length,беды,сироты,
   тёмных:тёмные.size,тёмныеБезИсточника,тёмныеВШколах,
 ...
 check('каждому умению Грани где-нибудь да учат',реестр.сироты.length===0,реестр.сироты);
 check('тёмным умениям на Грани не учат нигде — только за Гранью',
  реестр.тёмных===8&&реестр.тёмныеВШколах.length===0&&реестр.тёмныеБезИсточника.length===0,
  {тёмных:реестр.тёмных,вШколах:реестр.тёмныеВШколах,безИсточника:реестр.тёмныеБезИсточника});
 ...
 const дела=await page.evaluate(()=>{
  ...
  /* Корневой шаг переносит.
     Встаём там, откуда корни дотягиваются. Прежде проверка полагалась на то,
     что лес найдётся где угодно: рельеф выбирался для каждой клетки жребием, и
     лес был рассыпан повсюду. В связном мире лес — это область, и посреди степи
  ...
  const лесРядом=(()=>{
   ...
   for(let r=1;r<=600;r++)for(let dy=-r;dy<=r;dy++)for(let dx=-r;dx<=r;dx++){
    ...
    if(c&&["forest","swamp"].includes(c.terrain[0]))return {x,y};}
   return null;})();
  out.лесНайден=!!лесРядом;
  if(лесРядом){G.place=null;G.ship=null;
   G.x=Math.max(0,Math.min(WORLD-1,лесРядом.x+6));
   G.y=Math.max(0,Math.min(WORLD-1,лесРядом.y+6));}
  const бx=G.x,бy=G.y;
 ...
 check('корневой шаг переносит игрока в лес',
  дела.лесНайден===true&&дела.корни===true&&дела.перенёс===true,дела);
