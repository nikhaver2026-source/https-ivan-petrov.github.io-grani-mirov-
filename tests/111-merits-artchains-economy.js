...
(async()=>{
 ...
 check('десять звеньев артефактной цепочки, десять редкостей артефактов, семь полей энциклопедии на сотнях записей; самопроверка мира видит достижения',
  данные.steps===10&&данные.ranks===10&&данные.fields===7&&данные.записей>100&&данные.modules&&данные.selfcheck===true,данные);
 ...
 check('достижение берётся само, когда мир дорос: звучит по роду, говорит, пишется в летопись и реестр; второй раз не берётся; прогресс и строка видны',
  заслуги.нет===false&&заслуги.прогресс0===0&&заслуги.взято>=1&&заслуги.есть&&заслуги.звук&&заслуги.летопись&&заслуги.сказано.length>0&&заслуги.реестр&&заслуги.второй===0&&заслуги.ещё>=1&&заслуги.чары&&/Взято в день/.test(заслуги.строка)&&/Достижений \d+ из/.test(заслуги.text)&&заслуги.поРоду>=2,заслуги);
 ...
 check('у торговца склад, поставщик, конкурент, долг, предпочтение и предел казны; цена слушает предпочтение; склад тает от покупок и пополняется по дням; строка видна у торговца и молчит у прочих',
  лавка.поля.length===0&&лавка.дешевле&&лавка.убыло&&лавка.подвоз&&/склад \d+ из \d+/.test(лавка.text)&&/подвоз — /.test(лавка.text)&&/list-line/.test(лавка.line)&&лавка.неТорговец===""&&лавка.цены[0]>0&&лавка.цены[1]>0,лавка);
