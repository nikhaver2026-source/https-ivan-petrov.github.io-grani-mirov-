/* ═══════════════════════════════════════════════════════════════════════
  ...
  2в. Мягкость на конце слова и перед согласным не теряется: «Грань
      Миров», а не «Гран Миров»; «конь», «кровь», «соль», «только»,
      «деньги» — и в словаре, и в разборе незнакомого слова.
...
(async()=>{
 ...
 /* Мягкость: eSpeak, эталон записи голоса, теряет её на конце слова и перед
    согласным, и голос говорил «Гран Миров». Теперь она на месте. */
 const мягко=await page.evaluate(()=>new Promise(ok=>{
  const w=Speech._grani.worker;
  const h=e=>{if(e.data&&e.data.type==="phonemes"&&e.data.id==="мягко"){w.removeEventListener("message",h);ok(e.data.parts);}};
  w.addEventListener("message",h);w.postMessage({type:"phonemes",id:"мягко",text:"Добро пожаловать в Грань Миров. Конь, кровь, соль, только деньги. Тарань и Мираль."});
  setTimeout(()=>ok(null),5000);}));
 const мягкоСтрока=(мягко||[]).join(" ");
 check('2в. мягкость на конце слова и перед согласным на месте: «Грань», «конь», «кровь», «соль», «только», «деньги», и у незнакомых слов',
  /ɡrˈɑnʲ /.test(мягкоСтрока)&&/kˈonʲ,/.test(мягкоСтрока)&&/krˈofʲ,/.test(мягкоСтрока)&&/sˈolʲ,/.test(мягкоСтрока)
  &&/tˈoɭʲkʌ/.test(мягкоСтрока)&&/dʲˈenʲɡʲɪ/.test(мягкоСтрока)&&/tˈɑrʌnʲ /.test(мягкоСтрока)&&/mʲirˈɑlʲ\./.test(мягкоСтрока),мягко);
 const незнакомое=(разбор[3]||[])[0]||"";
 check('3. слово, которого нет в словаре, всё равно получает звуки и ударение',/kʲ.*ɭʲv.*ˈ/.test(незнакомое)&&/prʲiʃˈoɭ/.test(незнакомое),незнакомое);
