/* ═══════════════════════════════════════════════════════════════════════
   ...
   2. Обычное касание двумя пальцами в окне по-прежнему его закрывает.
...
(async()=>{
 ...
 /* ── 2. обычное касание двумя пальцами закрывает окно ── */
 await настройки();await сброс();
 await рука(2,0,0,{вразнобой:30,x:150,y:300});
 const д2=await снять();
 check('2. быстрое касание двумя пальцами в окне закрывает его, как закрывало',
  д2.жур.some(x=>/handleTwoFingerTap/.test(x))&&д2.слой===null,д2);
