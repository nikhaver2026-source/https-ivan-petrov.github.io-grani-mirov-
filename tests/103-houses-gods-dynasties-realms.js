/* ═══════════════════════════════════════════════════════════════════════
   ...
   1. Двадцать четыре Дома: двенадцать света по восьми державам и двенадцать
      тёмных по восьми тёмным империям; у каждого девиз, герб, гербовый
   ...
   4. Малые боги: двадцать по областям и восемь по тёмным империям — вместе
      тридцать два и тридцать пять; у каждого сфера, культ, святое место,
   ...
   5. Династии восьми держав и восьми тёмных империй: правитель, наследники,
...
(async()=>{
 ...
 check('Дома света покрывают все восемь держав, тёмные — все восемь империй; каждая клетка света принадлежит какому-то Дому и все двенадцать встречаются',
  дома.держСвет===8&&дома.держТьма===8&&дома.покрытиеСвет===12&&дома.пустоСвет===0&&дома.своя,{покрытие:дома.покрытиеСвет,пусто:дома.пустоСвет});
 ...
 const боги=await page.evaluate(async()=>{
  ...
  G.day=g.праздник+31;const w2={};Gods.weather(w2);r.погодаБудня=w2[g.погода]||0;
  r.цена={праздник:Gods.priceK(g.цена.res,g.праздник+30),будни:Gods.priceK(g.цена.res,g.праздник+31)};
  G.day=g.праздник+30;r.мана=Gods.manaDiscount({n:"x",школа:g.школа,school:g.школа,custom:true});
  ...
  if(храм){G.x=храм.x;G.y=храм.y;G.place=null;G.ship=null;
   ...
   r.пантеон={строк:el.querySelectorAll(".list-line").length-1,шапка:el.textContent.includes("32")&&el.textContent.includes("35"),молиться:el.querySelectorAll("button").length>0};
   while(activeLayer())closeTopUI();}
  ...
  return r;});
 check('двадцать восемь малых богов: двадцать по областям света, восемь по тёмным империям; с пантеоном и тёмными богами — тридцать два и тридцать пять',
  боги.всего===28&&боги.свет===20&&боги.тьма===8&&боги.счёт.свет===32&&боги.счёт.тьма===35&&боги.области===20&&боги.империи===8,боги.счёт);
 check('у каждого малого бога все поля, голос записью, свой день праздника, настоящая погода, школа и враг',
  !боги.поля.length&&!боги.нетЗаписи.length&&боги.праздники===28&&!боги.погода.length&&!боги.школы.length&&!боги.враги.length,{поля:боги.поля,записи:боги.нетЗаписи,погода:боги.погода,враги:боги.враги});
 check('у каждой области и каждой тёмной империи свой бог этих мест',боги.местные===20&&боги.тёмныеМестные===8,{свет:боги.местные,тьма:боги.тёмныеМестные});
 ...
 check('пантеон показывает раздел малых богов: двадцать восемь строк, счёт тридцать два и тридцать пять, кнопки молитвы в храме',
  боги.пантеон&&боги.пантеон.строк===28&&боги.пантеон.шапка&&боги.пантеон.молиться,боги.пантеон);
 ...
 const держ=await page.evaluate(async()=>{
  ...
  G.x=было.x;G.y=было.y;G.dark=было.dark;
  return r;});
 check('восемь династий держав и восемь тёмных: правитель, три наследника, оппозиция, шпионы, раздор, три–пять школ чар, войско, товар, зал записью; имена династий не повторяются',
  держ.свет===8&&держ.тьма===8&&держ.полныеСвет&&держ.полныеТьма&&держ.династии===16,{св:держ.полныеСвет,т:держ.полныеТьма,д:держ.династии});
 ...
 check('окно политики: восемь династий, восемь тёмных империй, восемь малых королевств, двадцать четыре Дома',
  держ.окно.династии===8&&держ.окно.тьма===8&&держ.окно.королевства===8&&держ.окно.дома===24,держ.окно);
