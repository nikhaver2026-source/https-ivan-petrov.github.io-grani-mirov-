...
(async()=>{
 ...
 /* Закачка записей в память браузера идёт своим чередом и сама говорит о
    себе: это нужно игроку и к касанию отношения не имеет. Проверяется, что
    молчит САМО касание, а не весь мир вокруг. */
 const речьЧисто=речь1.filter(t=>!/Скачано \d+ процент|Записи скачаны|Загрузка записей/.test(String(t)));
 check('одно касание одним пальцем не собирает и молчит',
  послеОдного.length===0&&речьЧисто.length===0,{инвентарь:послеОдного,сказано:речьЧисто.slice(0,2)});

 /* ── 6. Двойное касание по полю ничего не делает ── */
 await page.evaluate(()=>{G.inv={};});
 await clearSaid();
 await doubleTap(195,470);
 const послеДвойного=await page.evaluate(()=>Object.entries(G.inv).filter(([,v])=>v>0));
 const речьДвойного=(await said()).filter(t=>/Собрано/.test(String(t)));
 /* Двойное касание поля — действие здесь (набор 176): на ресурсе оно
    собирает, и ровно одну единицу. */
 /* Сбор может дать и две единицы (удача, навык, вьючный скакун): проверяется,
    что он случился ровно один раз, а не сколько принёс. */
 check('двойное касание по полю — действие здесь: на ресурсе сбор срабатывает ровно один раз',
  послеДвойного.length===1&&послеДвойного[0][1]>=1&&речьДвойного.length===1,{инвентарь:послеДвойного,сбор:речьДвойного.length});
