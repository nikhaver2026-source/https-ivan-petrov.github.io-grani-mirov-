...
(async()=>{
 ...
 const работа=await page.evaluate(()=>{
  ...
  for(const t of TECHS){
   ...
   /* Припас берётся из самих работ, а не из списка сырья: с появлением
      переделов (§12) половина техник ест не породу, а полуфабрикат — и
      оставленный вручную список молча делал их «нечем работать». */
   TECHS.forEach(x=>{for(const k in (x.берёт||{}))if(!(k in G.inv))G.inv[k]=20;});
   ...
   /* Передел меняет припас один к одному: две руды уходят, две дроблёной
      приходят — общий счёт котомки не сдвигается, а плата взята. Поэтому
      входное сырьё считается отдельно от общего счёта. */
   const вход=()=>Object.keys(t.берёт||{}).reduce((s,k)=>s+(Number(G.inv[k])||0),0);
   const доВход=вход();
   ...
   итоги.push({id:t.id,ок:ок===true,
    припасУшёл:после.инв<доИнв||вход()<доВход,
