...
(async()=>{
 ...
 /* Три работы припаса не берут и брать не должны: разбор вещи, разбор письма
    и прочтение морока. Смотреть и читать — не тратить. Времени они стоят
    наравне со всеми, и это здесь и проверяется; всё остальное обязано брать. */
 check('припас берут все работы, кроме разбора и прочтения; времени стоит всякая',
  работа.безПрипаса.slice().sort().join(",")==="analyze,decipher,unveil"
