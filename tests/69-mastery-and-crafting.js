...
(async()=>{
 ...
 check('техник не меньше девяти, и у каждой мастерство, станок и живые записи',
  таблицы.техник>=9&&таблицы.плохиеТ.length===0,таблицы);
 ...
 check('все техники срабатывают у своего станка',
  работа.проверено>=9&&работа.неСделались.length===0,работа.неСделались);
 /* Разбор вещи припаса не берёт и брать не должен: смотреть — не тратить.
    Времени он стоит наравне со всеми, и это здесь и проверяется. */
 check('каждая работа, кроме разбора вещи, берёт припас; времени стоит всякая',
  работа.безПрипаса.join(",")==="analyze"&&работа.безВремени.length===0,
