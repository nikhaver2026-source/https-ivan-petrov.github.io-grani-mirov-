...
(async()=>{
 ...
 /* Пять работ припаса не берут, и каждая по своей причине. Разбор вещи,
    разбор письма и прочтение морока — потому что смотреть и читать не значит
    тратить. «Собрать гомункула» и «Обряды демонолога» — потому что это не
    работа у станка, а ДВЕРЬ в свою систему: там свой выбор частей, свои
    требования и свой припас, и берётся он там, а не здесь. Времени стоит
    всякая работа, и это здесь тоже проверяется. */
 check('припас берут все работы, кроме разбора, прочтения и двух дверей; времени стоит всякая',
  работа.безПрипаса.slice().sort().join(",")==="analyze,decipher,rites,unveil,vessel"
