...
(async()=>{
 ...
 const кнопка=await page.evaluate(()=>{
  ...
  const t=soundTotals();
  /* С 3.6 кнопка называет только себя; число звуков — во всплывающей
     подсказке и в самом каталоге. */
  const m=(String(b.title||"").match(/\d+/)||[])[0];
  return {написано:Number(m),настоящее:t.всего,текст:b.textContent,речь:b.dataset.speak};});
 check('кнопка каталога называет себя одним именем, а число звуков — настоящее',
  кнопка.текст==="Энциклопедия звуков"&&кнопка.речь==="Энциклопедия звуков"
