...
(async()=>{
 ...
 /* ── 4. Пол под ногой ──
    Версия 3.4: в одном месте пол один на весь ярус (жалоба игрока — шаги в
    одном месте менялись, а в разных звучали одинаково). */
 const пол=await page.evaluate(()=>{
  ...
  const всеВиды={};Object.keys(PLACE_FLOOR).forEach(v=>всеВиды[v]=набор(v,0));
  const глубины={};[1,2,3,4,5].forEach(d=>глубины[d]=набор("ruins",d));
  /* Все поверхности, что встречаются, обязаны иметь имя и звук. */
  const все=new Set();
  Object.values(всеВиды).forEach(a=>a.forEach(x=>все.add(x)));
  ...
  /* Подвалы одной глубины в разных местах. */
  const подвалы=new Set();
  for(let i=0;i<12;i++)подвалы.add(набор("ruins",2,1+i*9,3+i*4).join());
  /* Каждой поверхности своя запись: одинаковую роль делят только
     поверхности, у которых свой второй слой. */
  const поРоли={};Object.entries(SURF_ROLE).forEach(([k,r])=>{(поРоли[r]=поРоли[r]||[]).push(k);});
  const делят=Object.entries(поРоли).filter(([r,ks])=>ks.length>1&&ks.filter(k=>!STEP_LAYER[k]).length>1).map(([r,ks])=>r+":"+ks.join("/"));
  G.place=null;
  return {наборы,глубины,безИмени,безЗвука,всего:все.size,смеси:DEPTH_MIX,делят,
   разныеПодвалы:подвалы.size,
 ...
 check('в одном месте пол один: шаг по всему ярусу одинаковый',
  Object.values(пол.наборы).every(a=>a.length===1)&&Object.values(пол.глубины).every(a=>a.length===1),пол.наборы);
 check('восемь видов построек различаются полом',
  пол.различимы===8,{различимо:пол.различимы});
 check('глубина слышна по полу: порода яруса — из смеси своей глубины, хрусталя наверху нет',
  [1,2,3,4,5].every(d=>пол.смеси[d].includes(пол.глубины[d][0]))&&пол.глубины[1][0]!=="crystal"&&
  new Set([1,2,3,4,5].map(d=>пол.глубины[d].join())).size>=3,пол.глубины);
 check('подвалы одной глубины в разных местах звучат по-разному',
  пол.разныеПодвалы>=2,{разных:пол.разныеПодвалы});
 check('у каждой поверхности своя запись: общую делят только те, кого различает второй слой',
  пол.делят.length===0,пол.делят);
 ...
 check('каждая держава учит своему набору чар',
  учение.держав===12&&учение.разных===12&&учение.нет.length===0,учение);
 ...
 check('у каждой из двенадцати держав своё поручение со своим поводом',
  наряды.держав===12&&наряды.разных===12&&наряды.наборов===12&&наряды.беды.length===0,
