...
(async()=>{
 ...
 const цена=await page.evaluate(()=>{
  ...
  r.обычное=smuggleCheck("руда",100);
  return r;});
 ...
 check('2в. цена объясняет себя словами: запрет и пошлина названы; контрабанда при жребии бьёт штрафом и войско это помнит, преступный мир ценит',
  /контрабанда/.test(цена.почему)&&/пошлина/.test(цена.почемуРуда)&&/Штраф/.test(цена.штраф)&&цена.золото<1000&&цена.крим>0&&цена.войско<0&&/из-под полы/.test(цена.безШтрафа)&&цена.золото2===1000&&цена.обычное==="",
 ...
 check('3. досье державы несёт все поля §124, союзники и враги совпадают с отношениями, текст и окно политики его показывают',
  !досье.поля.length&&досье.цели>=1&&/ /.test(досье.правитель)&&досье.союзВерно&&досье.врагиВерно&&/Правитель:/.test(досье.текст)&&/Тайное дело седмицы/.test(досье.текст)&&/Внутренний раздор/.test(досье.текст)&&/Под запретом/.test(досье.текст)&&/Менять его можно/.test(досье.текст)&&/Всего держав 8/.test(досье.всего)&&досье.окно===8,
