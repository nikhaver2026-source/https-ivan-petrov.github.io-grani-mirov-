/* ═══════════════════════════════════════════════════════════════════════
  ...
  10. Инвентарь открывается списком всех двадцати одного раздела по
...
(async()=>{
 ...
 check('1. настройки открываются списком из семи пунктов по порядку, настроек на первом уровне не видно, пункт называет, что в нём',
  ...
  &&список.подписи.every(t=>/\d+ настро/.test(t)&&/Двойное касание — открыть/.test(t))
  &&/^Синтезатор речи: /.test(список.подписи[1])&&/^Звуки: /.test(список.подписи[0]),список);
 ...
 check('3. свайп до «Синтезатора речи» и двойное касание: в окне только его настройки, выбор на первой, пункт назван',
  ...
  &&открыт.курсор==="setTtsEngine"&&открыт.сказано.some(t=>/Пункт «Синтезатор речи»/.test(t)),открыт);
 ...
 check('4. свайп двумя пальцами вниз возвращает к списку на тот же пункт, окно открыто; второй свайп закрывает окно',
  ...
  &&кСписку.сказано.some(t=>/Ко всем пунктам настроек/.test(t))&&закрыто,{кСписку,закрыто});
 ...
 check('7. в пункте из одного раздела нет ни лишнего заголовка, ни строки перескока; в пункте из двух они есть',
  один.головВидно===0&&!один.строка&&/Синтезатор речи/.test(один.подсказка)
 ...
 /* ── 10. инвентарь: все двадцать один раздел ── */
 await закрытьВсё();
 const разделы=await page.evaluate(()=>{
  ...
  return {ids:п.map(x=>x.dataset.punkt),порядок:INV_CATS.map(c=>c.id),
   пустых:п.filter(x=>/: пусто\./.test(x.dataset.speak)).length,
   ресурсы:(п.find(x=>x.dataset.punkt==="res")||{dataset:{}}).dataset.speak,
   сокровища:(п.find(x=>x.dataset.punkt==="treasure")||{dataset:{}}).dataset.speak,
   вещейВидно:cursorItems(m).filter(x=>/^invpick:/.test(x.dataset.cmd||"")).length};});
 check('10. инвентарь открывается списком всех двадцати одного раздела по порядку: пустые — «пусто», полные — с числом, вещей на первом уровне не видно',
  разделы.ids.length===21&&разделы.ids.join(",")===разделы.порядок.join(",")&&разделы.пустых===21-6
  &&/^Ресурсы: 2 вида, всего 6 штук\./.test(разделы.ресурсы)&&/^Сокровища: 1 вещь\./.test(разделы.сокровища)
  &&разделы.вещейВидно===0,разделы);
 ...
 check('11. двойное касание по «Ресурсам» открывает только ресурсы; свайп двумя пальцами вниз возвращает, выбор на «Ресурсах»',
  дошлиДоРесурсов&&ресурсы.раздел==="res"&&ресурсы.вещи.sort().join(",")==="дерево,руда"
 ...
 const выбор=await page.evaluate(()=>{
  ...
  return {был,послеНазад,снова,вДругом,ответ};});
 check('13. выбор вещи принадлежит открытому разделу: вернулись к списку или открыли другой раздел — выбор снят, действия не открываются',
  выбор.был==="руда"&&выбор.послеНазад===null&&выбор.снова==="руда"&&выбор.вДругом===null
  &&/Сначала выберите вещь/.test(выбор.ответ),выбор);
 await закрытьВсё();
 ...
 const добыча=await page.evaluate(()=>{
  ...
  const ждём={weapon:["Меч","Кинжал","Посох"],shield:["Щит"],armor:["Кольчуга"],helm:["Шлем"],acc:["Плащ","Кольцо","Амулет"]};
  ...
  r.окно={сокровища:речь("treasure"),магия:речь("magic"),ингредиенты:речь("ingredient")};
  closeTopUI();
 ...
 check('14. подобранное и полученное в награду ложится в свой раздел: снаряжение по роду, ремесло начала, веха глубины, сокровище, награда дела, трофей владыки, сырьё, травы, органы, рыба, зелье, артефакт, вложенная чара',
  ...
  &&/^Сокровища: 3 /.test(добыча.окно.сокровища)&&/^Магические предметы: 1 /.test(добыча.окно.магия)
