...
(async()=>{
 ...
 check('10. инвентарь открывается списком всех шестнадцати разделов по порядку: пустые — «пусто», полные — с числом, вещей на первом уровне не видно, последняя строка — что надето',
  разделы.ids.length===16&&разделы.ids.join(",")===разделы.порядок.join(",")&&разделы.пустых===16-5
  &&/^Ресурсы: 3 вида, всего 8 штук\./.test(разделы.ресурсы)&&/^Артефакты: 1 вещь\./.test(разделы.сокровища)
 ...
 check('14. подобранное и полученное в награду ложится в свой раздел: снаряжение по роду, ремесло начала, веха глубины, сокровище, награда дела, трофей владыки, сырьё, травы, органы, рыба, зелье, артефакт, вложенная чара',
  ...
  &&добыча.окно.ждёмАрт>=5&&new RegExp("^Артефакты: "+добыча.окно.ждёмАрт+" ").test(добыча.окно.артефакты)
  &&new RegExp("^Ресурсы: "+добыча.окно.ждёмРес+" ").test(добыча.окно.ресурсы)
