/* ════════════════════════════════════════════════════════════════════════
   ...
   дна. Теперь там двенадцать империй, которые воюют и заключают союзы куда злее,
...
(async()=>{
 ...
 check('двенадцать тёмных империй, десять кланов и шесть школ',
  реестр.империй===12&&реестр.кланов===10&&реестр.школ===6,реестр);
 ...
 check('воюют все двенадцать империй, и причин у войн много',
  политика.воевали===12&&политика.причин>=8,политика);
 ...
 const мир=await page.evaluate(()=>{
  ...
  /* Сетка в четыреста точек проходила мимо угловых владений: там земли
     немного, и решётка её просто не задевала. Шаг в двадцать пять клеток
     покрывает карту целиком. */
  for(let x=0;x<2000;x+=25)for(let y=0;y<2000;y+=25)империи.add(darkEmpireAt(x,y).id);
 ...
 check('все двенадцать империй занимают свои земли',мир.империй===12,мир);
 ...
 const глубина=await page.evaluate(()=>{
  ...
  /* Глубже сотого хода нет ни за Гранью, ни на Грани: теперь и там сто
     ярусов, но круги у них разные. */
  ...
  const светКруги=[];for(let d=1;d<=100;d+=10)светКруги.push(lightCircle(d).n);
  const тьмаКруги=[];for(let d=1;d<=100;d+=10)тьмаКруги.push(darkCircle(d).n);
  const общих=светКруги.filter(n=>тьмаКруги.indexOf(n)>=0).length;
  return {кругов:круги.size,полов:полы.size,откликов:отклики.size,вес,пределТьма,пределСвет,
   светКругов:new Set(светКруги).size,общих};});
 ...
 check('сто ярусов и за Гранью, и на Грани',
  глубина.пределТьма===100&&глубина.пределСвет===100,глубина);
 check('круги Грани не повторяют кругов тьмы',
  глубина.светКругов===10&&глубина.общих===0,
  {светКругов:глубина.светКругов,общих:глубина.общих});
