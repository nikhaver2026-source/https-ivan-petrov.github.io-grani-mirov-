...
(async()=>{
 ...
 /* Верхнеуровневая функция, объявленная дважды, затирает первую так же
    молча, как ключ объекта: так своя seasonNow подменила мировую, и весь
    мир разом потерял поры — растения, погоду, зверьё и сорта ресурсов. */
 const дублиФункций=(()=>{const было={},дубли=[];
  (src.match(/^function [A-Za-zА-Яа-яЁё_$][\w$]*\s*\(/gm)||[]).forEach(l=>{
   const n=l.slice(9).replace(/\s*\($/,"").trim();
   if(было[n])дубли.push(n);else было[n]=1;});
  return {функций:Object.keys(было).length,дубли};})();
 ...
 check('ни один ключ CMD, ни одна верхнеуровневая функция и ни одна строка меню действий не объявлены дважды, и у каждой строки меню есть своя команда',
  повторы.дубли.length===0&&повторы.ключей>=240&&дублиФункций.дубли.length===0&&дублиФункций.функций>=300&&
  строкиМеню.дубли.length===0&&строкиМеню.безКоманды.length===0&&строкиМеню.строк>=90,
  {ключей:повторы.ключей,дубли:повторы.дубли,функций:дублиФункций.функций,дублиФункций:дублиФункций.дубли,
   меню:строкиМеню.дубли,безКоманды:строкиМеню.безКоманды,строк:строкиМеню.строк});
