...
(async()=>{
 ...
 check('врата сети стоят у каждой из двенадцати держав',
  узлы.врат===12&&узлы.имена.every(n=>/^Врата державы/.test(n||"")),узлы.имена);
 ...
 const цены=await page.evaluate(()=>{
  ...
  /* Нейтралитет ищется нарочно: при двенадцати державах взятая наугад пара
     может оказаться союзной, и тогда «союз дешевле нейтралитета» мерило бы
     союз против союза. Отношение складывается из основы, дрейфа и поправки,
     поэтому нулевая поправка нейтралитета не гарантирует — подбираем ту,
     при которой пара и вправду нейтральна. */
  let пол0=null;
  for(const н of [0,-30,30,-60,60,-90,90,-120,120]){
   G.diplo={};G.diplo[diploKey(i,j)]=н;
   const f=portalFare(a,цель);
   if(f.слово==="нейтралитет"){пол0=f;break;}}
  if(!пол0){G.diplo={};G.diplo[diploKey(i,j)]=0;пол0=portalFare(a,цель);}
