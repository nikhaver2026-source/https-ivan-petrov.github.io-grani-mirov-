...
(async()=>{
 ...
 const логова=await page.evaluate(()=>{
  ...
  for(let i=0;i<60000;i++){
   ...
   if(!вЛесу)continue;
   const c=safeFn(()=>cellContent(x,y),null);
   const он=!!(c&&c.monster&&c.monster.id==="wolf");
   if(p>0.75){бл++;if(он)близко++;}
   else if(p<0.15){дл++;if(он)далеко++;}}
  return {бл,дл,уЛогова:Math.round(близко/Math.max(1,бл)*1000)/10,
   наКраю:Math.round(далеко/Math.max(1,дл)*1000)/10};});
 check('выборка у логова и на краю удела набралась',
  логова&&логова.бл>=50&&логова.дл>=50,логова);
 check('у логова тварь встречается чаще, чем на краю удела',
  логова&&логова.уЛогова>логова.наКраю*1.5,логова);
