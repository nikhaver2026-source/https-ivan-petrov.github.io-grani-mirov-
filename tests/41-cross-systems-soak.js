...
(async()=>{
 ...
 const прогон=await page.evaluate(async()=>{
  ...
  /* Шаг обязан ОТВЕТИТЬ — но ответ шага не всегда слово. Снаружи сторона
     называется только при повороте: иначе «север» повторялся бы на каждом
     шаге и забивал бы собой постройки, ресурсы и тварей, а вместо слова там
     звучит сам шаг. Поэтому годным ответом считаем и речь, и то, что герой
     действительно сдвинулся с места. Молчание при неподвижном герое — вот
     это и есть зависшая игра. */
  window.__said=[];
  const доШага={x:G.x,y:G.y,px:G.place?G.place.x:null,py:G.place?G.place.y:null};
  тихо(()=>{G.place?moveInside("N"):move("N");});
  const сдвинулся=G.place
   ?(G.place.x!==доШага.px||G.place.y!==доШага.py)
   :(G.x!==доШага.x||G.y!==доШага.y);
 ...
 const крест=await page.evaluate(async()=>{
  ...
  /* Поиск идёт от середины мира — там же, где стоит герой. Прежде здесь
     стояла тысяча: в мире два на две тысячи это была середина, а в нынешнем
     стало углом, и до половины видов приходилось идти впустую.

     И круг сужен с 220 до 90. Часть видов — палаты Тёмных земель, каких в
     светлом мире нет вовсе: за ними обход доходил до края круга и прочёсывал
     сто девяносто четыре тысячи клеток ВПУСТУЮ, и так для каждого состояния.
     Отсюда и брались минуты. Девяносто клеток кругом — тридцать три тысячи
  ...
  const найти=(вид)=>{
   ...
   for(let r=1;r<90;r++)for(let dy=-r;dy<=r;dy++)for(let dx=-r;dx<=r;dx++){
    ...
    if(c&&c.structure&&c.structure.type===вид)return {x:C+dx,y:C+dy,c};}
   return null;};
  const состояния={
   "в поле":()=>{базово();},
  ...
  for(const [имя,поставить] of Object.entries(состояния)){
   for(const к of команды){
    ...
    /* Заставка отвечает самой записью — музыкой и голосом, а не речью игры.
       Запущенную обрываем, чтобы следующее положение начиналось с тишины. */
    const заставка=(к==="intro"&&!!Intro.el);
    if(к==="intro")Intro.finish(true);
    if(!молчаНарочно&&!заставка&&!window.__said.join("").trim()&&!activeLayer())медлили.push([к,имя]);
    const б=window.__законы();
