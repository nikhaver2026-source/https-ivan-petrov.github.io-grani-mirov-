...
(async()=>{
 ...
 check('субтитры по уровням: выключены — ничего; важные — только важное; реплики — важное и реплики; все звуки — и описание записи',
  режимы.off[0]===0&&режимы.off[1]===false&&режимы.off[2]===true&&режимы.imp.join()==="true,false,false"&&режимы.sp.join()==="true,false"&&режимы.snd[0]===true&&режимы.snd[1]===true&&/\S/.test(режимы.snd[2]),режимы);
 ...
 check('карта жестов: пятнадцать записей на четыре числа пальцев (раздельное касание убрано в 3.9), без споров, читается словами',жесты.n===15&&жесты.conf.length===0&&жесты.fingers==="1234"&&жесты.text>500,жесты);
