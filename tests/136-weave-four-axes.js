...
(async()=>{
 ...
 check('каждая из тридцати восьми школ несёт ровно одну стихию, и ни одна не дважды',
 ...
 check('все сто четырнадцать именных заклинаний школ творятся и ни одно не пустое',
  школьные.всего===114&&школьные.плохих===0&&школьные.безЯдра.length===0,школьные);
 ...
 const окно=await page.evaluate(()=>{
  ...
  G.place=null;
  return r;});
 check('окно составителя показывает все четыре оси: 38 школ, 12 форм, 16 ядер, 18 стихий, 16 усилений',
  !окно.нет&&окно.школ===38&&окно.форм===12&&окно.усил===16
