...
(async()=>{
 ...
 check('сто два заклинания, ровно по три у каждой из тридцати четырёх школ',
  состав.всего===102&&состав.школ===34&&состав.неПоТри.length===0,состав);
 ...
 check('все сто два попали в общий список чар и творятся тем же слотом',
  вСписке.нашлись===102&&вСписке.сCustom===102,вСписке);
 ...
 const окно=await page.evaluate(()=>{
  ...
  return {нет:false,кнопок:кнопки.length,
   ...
   естьСвод:/Чар школ 102/.test(el.innerHTML)};});
 check('в окне Академии есть раздел со всеми тридцатью четырьмя школами и сводом',
  !окно.нет&&окно.школВТексте===34&&окно.естьСвод,окно);
