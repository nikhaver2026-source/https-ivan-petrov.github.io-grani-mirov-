...
(async()=>{
 ...
 const состав=await page.evaluate(()=>({
  ...
  случайныхБезСопряжения:RANDOM_EVENTS.filter(e=>e.id.indexOf("conj_")!==0).length}));
 ...
 check('два события Сопряжения сверх четырнадцати обычных, и оба со звуком',
  состав.событий===2&&состав.событияСоЗвуком&&состав.случайныхБезСопряжения===14,состав);
