...
(async()=>{
 ...
 check('1. двадцать семь школ пяти семейств с силой и звуками из банка; все исходные заклинания приписаны к школе',
  школы.n===27&&школы.уник===27&&школы.семейств===5&&школы.плохие.length===0&&школы.исходные.length===12&&школы.безШколы.length===0,
 ...
 const окно=await page.evaluate(()=>{
  ...
  closeMagicPanel();
  return r;});
 check('5а. окно составителя: 27 школ, 5 форм, 5 усилений и «Собрать»; выбор озвучен и отмечен',
  окно.открыто&&окно.видно&&окно.школ===27&&окно.форм===5&&окно.усил===5&&окно.собрать&&/Огонь|Выйдет/.test(окно.сказШкола)&&/Выйдет «Стрела огня»/.test(окно.сказФорма)&&окно.галочка,
 ...
 check('6г. «Школы» перечисляют двадцать семь школ и известные чары по школам',
  /Школ заклинаний 27/.test(каст.школыТекст)&&/Огонь — /.test(каст.школыТекст)&&каст.записей===5,каст.школыТекст.slice(0,160));
