...
(async()=>{
 ...
 const слышалСедьмые=await новости({rate:5,rateFast:1,newsV:7});
 const слышалВосьмые=await новости({rate:5,rateFast:1,newsV:8});
 const слышалДевятые=await новости({rate:5,rateFast:1,newsV:9});
 const слышалДесятые=await новости({rate:5,rateFast:1,newsV:10});
 const NV=слышалДесятые.newsV;
 const слышалОдиннадцатые=await новости({rate:5,rateFast:1,newsV:11});
 ...
 check('вернувшийся игрок один раз слышит, что изменилось, и отметка ложится в сохранение',
  вернулся.сказано===1&&вернулся.newsV===NV&&/Меню действий теперь открывается списком разделов/.test(вернулся.целиком)
 ...
 check('слышавший прошлые новости слышит только новые: про разделы и звук, без пересказа старого',
  слышалПрошлые.сказано===1&&слышалПрошлые.newsV===NV&&/списком разделов/.test(слышалПрошлые.целиком)
 ...
 check('слышавший третьи новости слышит про котомку и меню, без пересказа разделов и звука',
  слышалТретьи.сказано===1&&слышалТретьи.newsV===NV&&/шестнадцати разделов/.test(слышалТретьи.целиком)
 ...
 check('слышавший четвёртые новости слышит про меню действий пунктами и про звук, без котомки',
  слышалЧетвёртые.сказано===1&&слышалЧетвёртые.newsV===NV&&/Меню действий теперь открывается списком разделов/.test(слышалЧетвёртые.целиком)
 ...
 check('слышавший пятые новости слышит только про музыку мест и живые звуки вех',
  слышалПятые.сказано===1&&слышалПятые.newsV===NV&&/Музыка теперь звучит только темой места/.test(слышалПятые.целиком)
 ...
 check('слышавший шестые новости слышит только про подвижный бой',
  слышалШестые.сказано===1&&слышалШестые.newsV===NV&&/Бой теперь подвижный/.test(слышалШестые.целиком)
 ...
 check('слышавший седьмые новости слышит только про обрыв речи, стороны и звук',
  слышалСедьмые.сказано===1&&слышалСедьмые.newsV===NV&&/обрывает речь/.test(слышалСедьмые.целиком)
  &&/Стороны света/.test(слышалСедьмые.целиком)&&!/Бой теперь подвижный/.test(слышалСедьмые.целиком),{начало:слышалСедьмые.пропечатано});
 check('слышавший восьмые новости слышит только про голоса жителей',
  слышалВосьмые.сказано===1&&слышалВосьмые.newsV===NV&&/Жители говорят громче/.test(слышалВосьмые.целиком)
  &&/Голоса людей/.test(слышалВосьмые.целиком)&&!/обрывает речь/.test(слышалВосьмые.целиком),{начало:слышалВосьмые.пропечатано});
 check('слышавший девятые новости слышит только про двойное касание и тайный ход',
  слышалДевятые.сказано===1&&слышалДевятые.newsV===NV&&/двойное касание одним пальцем/.test(слышалДевятые.целиком)
  &&/Тайный ход/.test(слышалДевятые.целиком)&&!/Жители говорят громче/.test(слышалДевятые.целиком),{начало:слышалДевятые.пропечатано});
 check('слышавший десятые новости слышит только про одно касание, порядок реплик и речь жителей',
  NV>=12&&слышалДесятые.сказано===1&&/Одно касание больше никогда ничего не выполняет/.test(слышалДесятые.целиком)
  &&/сначала звучит ваша реплика/.test(слышалДесятые.целиком)&&/только по-русски/.test(слышалДесятые.целиком)
  &&!/Тайный ход/.test(слышалДесятые.целиком),{NV,начало:слышалДесятые.пропечатано});
 check('слышавший одиннадцатые новости слышит только про голоса телефона списком',
  слышалОдиннадцатые.сказано===1&&/Все голоса телефона — списком/.test(слышалОдиннадцатые.целиком)
  &&!/Одно касание больше никогда/.test(слышалОдиннадцатые.целиком),{начало:слышалОдиннадцатые.пропечатано});
