...
(async()=>{
 ...
 /* Сами выпуски целиком — в меню действий, последним пунктом «Что нового». */
 {
  ...
  check('«Что нового» — последний пункт меню действий перед «Закрыть», с числом новых',
   м.пункт==="am:whatsnew"&&м.закрыть==="am:close"&&/Что нового \(новых: 5\)/.test(м.подпись||""),м);
  check('окно «Что нового»: выпуски от нового к старому, новые помечены, пропущенные читаются подряд',
   м.открыто&&м.выпуски[0]==="newsread:fresh"&&м.выпуски[1]==="newsread:"+NV&&м.выпуски.length===NV&&м.новые===5
   &&/Чувствительность жестов/.test(м.последний)&&/Действие с сундуком/.test(м.подряд)&&/Все голоса телефона/.test(м.подряд),м);
