 ...
 /* Сами выпуски целиком — в меню действий, последним пунктом «Что нового». */
 {
  ...
  const м=await p3.evaluate(()=>{
   ...
   out.тексты={
    ...
    17:/Камнемёт/.test(т(17))&&/по башне/.test(т(17))&&/Нежить/.test(т(17))&&/призраки/.test(т(17))&&/взрыв/.test(т(17)),
    18:/заставка/.test(т(18))&&/без шва/.test(т(18))&&/Считать ступени на лестнице/.test(т(18))&&/не накладываются/.test(т(18)),
    19:/листании/.test(т(19))&&/ровно один щелчок/.test(т(19))&&/в настройках — сухой/.test(т(19)),
    20:/одним именем/.test(т(20))&&/Supertonic 3/.test(т(20))&&/женским голосом/.test(т(20))&&/Тридцать одна фраза/.test(т(20)),
    21:/четыреста семь реплик/.test(т(21))&&/Gemini/.test(т(21))&&/Шепчущий шепчет/.test(т(21))&&/тексты остались прежними/.test(т(21)),
    22:/заставка/i.test(т(22))&&/Раздельного касания больше нет/.test(т(22))&&/подземелья слышно/.test(т(22)),
    23:/голосом Gemini/.test(т(23))&&/синтезатор телефона/.test(т(23))};
   return out;});
  await p3.close();
  /* С 3.6 пункт называет только себя; число новых выпусков хранится при нём
     и звучит в самом окне. */
  check('«Что нового» — последний пункт меню действий перед «Закрыть», одним именем',
   м.пункт==="am:whatsnew"&&м.закрыть==="am:close"&&м.подпись==="Что нового"&&м.речь==="Что нового"&&м.свежих==="14",м);
  check('окно «Что нового»: выпуски от нового к старому, новые помечены, пропущенные читаются подряд',
   м.открыто&&м.выпуски[0]==="newsread:fresh"&&м.выпуски[1]==="newsread:"+NV&&м.выпуски.length===NV&&м.новые===14
   &&/Gemini/.test(м.последний)&&/Действие с сундуком/.test(м.подряд)&&/Все голоса телефона/.test(м.подряд),м);
