...
(async()=>{
 ...
 check('слышавший прошлые новости слышит только новые: про разделы и звук, без пересказа старого',
  слышалПрошлые.сказано===1&&слышалПрошлые.newsV===4&&/списком разделов/.test(слышалПрошлые.целиком)
 ...
 check('слышавший третьи новости слышит только про котомку: шестнадцать разделов и места на теле, без пересказа разделов и звука',
  слышалТретьи.сказано===1&&слышалТретьи.newsV===4&&/шестнадцати разделов/.test(слышалТретьи.целиком)
  &&/тринадцать мест/.test(слышалТретьи.целиком)&&!/списком разделов/.test(слышалТретьи.целиком)
 ...
 check('второй раз этого не говорят, а новому игроку — ни разу',
  ужеСлышал.сказано===0&&новичок.сказано===0&&новичок.newsV===4,{ужеСлышал,новичок});
