...
(async()=>{
 ...
 const ужеСлышал=await новости({rate:5,rateFast:1,newsV:8});
 const новичок=await новости(null);
 check('вернувшийся игрок один раз слышит, что изменилось, и отметка ложится в сохранение',
  вернулся.сказано===1&&вернулся.newsV===8&&/Меню действий теперь открывается списком разделов/.test(вернулся.целиком)
 ...
 check('слышавший прошлые новости слышит только новые: про разделы и звук, без пересказа старого',
  слышалПрошлые.сказано===1&&слышалПрошлые.newsV===8&&/списком разделов/.test(слышалПрошлые.целиком)
 ...
 check('слышавший третьи новости слышит про котомку и меню, без пересказа разделов и звука',
  слышалТретьи.сказано===1&&слышалТретьи.newsV===8&&/шестнадцати разделов/.test(слышалТретьи.целиком)
 ...
 check('слышавший четвёртые новости слышит про меню действий пунктами и про звук, без котомки',
  слышалЧетвёртые.сказано===1&&слышалЧетвёртые.newsV===8&&/Меню действий теперь открывается списком разделов/.test(слышалЧетвёртые.целиком)
 ...
 check('слышавший пятые новости слышит только про музыку мест и живые звуки вех',
  слышалПятые.сказано===1&&слышалПятые.newsV===8&&/Музыка теперь звучит только темой места/.test(слышалПятые.целиком)
 ...
 check('слышавший шестые новости слышит только про подвижный бой',
  слышалШестые.сказано===1&&слышалШестые.newsV===8&&/Бой теперь подвижный/.test(слышалШестые.целиком)
 ...
 check('слышавший седьмые новости слышит только про обрыв речи, стороны и звук',
  слышалСедьмые.сказано===1&&слышалСедьмые.newsV===8&&/обрывает речь/.test(слышалСедьмые.целиком)
 ...
 check('второй раз этого не говорят, а новому игроку — ни разу',
  ужеСлышал.сказано===0&&новичок.сказано===0&&новичок.newsV===8,{ужеСлышал,новичок});
