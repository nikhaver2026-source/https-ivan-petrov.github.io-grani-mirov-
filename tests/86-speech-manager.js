...
(async()=>{
 ...
 const ужеСлышал=await новости({rate:5,rateFast:1,newsV:5});
 const новичок=await новости(null);
 check('вернувшийся игрок один раз слышит, что изменилось, и отметка ложится в сохранение',
  вернулся.сказано===1&&вернулся.newsV===5&&/Меню действий теперь открывается списком разделов/.test(вернулся.целиком)
 ...
 check('слышавший прошлые новости слышит только новые: про разделы и звук, без пересказа старого',
  слышалПрошлые.сказано===1&&слышалПрошлые.newsV===5&&/списком разделов/.test(слышалПрошлые.целиком)
 ...
 check('слышавший третьи новости слышит про котомку и меню, без пересказа разделов и звука',
  слышалТретьи.сказано===1&&слышалТретьи.newsV===5&&/шестнадцати разделов/.test(слышалТретьи.целиком)
 ...
 check('слышавший четвёртые новости слышит только про меню действий пунктами',
  слышалЧетвёртые.сказано===1&&слышалЧетвёртые.newsV===5&&/Меню действий теперь открывается списком разделов/.test(слышалЧетвёртые.целиком)
 ...
 check('второй раз этого не говорят, а новому игроку — ни разу',
  ужеСлышал.сказано===0&&новичок.сказано===0&&новичок.newsV===5,{ужеСлышал,новичок});
