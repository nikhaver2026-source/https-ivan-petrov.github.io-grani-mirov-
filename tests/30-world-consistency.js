...
(async()=>{
 ...
 const D=await page.evaluate(()=>{
  ...
  const звук={
   ...
   /* Маяк звучит либо записью, либо синтезом. Беда — только когда нет ни того,
      ни другого: тогда объект под ногами просто молчит. Запись ищут там же,
      где её ищет игра: по роли из BEACON_ROLE, а не по имени маяка — имя
      маяка в банке ролей не значится никогда, и проверка держалась на одном
      синтезе. */
   маякНемой:[...new Set(Object.values(RESBEACON))]
    .filter(r=>!SOUND_BANK[BEACON_ROLE[r]]&&!BEACON_SAMPLE[r]&&!BEACONS[r]),
   маякТолькоСинтез:[...new Set(Object.values(RESBEACON))]
    .filter(r=>!SOUND_BANK[BEACON_ROLE[r]]&&!BEACON_SAMPLE[r]&&!!BEACONS[r]),
   /* Девять поздних ресурсов делили четыре чужих маяка: шкура звучала костью,
      эфир — кристаллом. У каждого ресурса свой маяк, у маяка — своя запись. */
   ресурсБезСвоегоМаяка:["шкура","орган","кровь","яд","эссенция","эфир",
    "осколок времени","тяжёлый минерал","рунный камень"]
    .filter(r=>!RESBEACON[r]||!SOUND_BANK[BEACON_ROLE[RESBEACON[r]]]||!BEACON_INFO[RESBEACON[r]]),
   маяковРазных:new Set(Object.values(RESBEACON)).size,
   ресурсовСМаяком:Object.keys(RESBEACON).length};
 ...
 check('в каталоге сорок шесть народов, у каждого заполнено досье',
  D.народы.всего===98&&!D.народы.пустыеПоля.length,{всего:D.народы.всего,пустые:D.народы.пустыеПоля});
 ...
 check('у каждой державы существующие бог, правящий народ и столица',
  D.державы.всего===12&&!D.державы.чужойБог.length&&!D.державы.чужойПравящий.length&&!D.державы.безСтолицы.length,
  D.державы);
 ...
 check('у каждого ресурса свой маяк со своей записью и своим именем',
  !D.звук.ресурсБезСвоегоМаяка.length&&D.звук.маяковРазных===17,
  {без:D.звук.ресурсБезСвоегоМаяка,маяков:D.звук.маяковРазных,ресурсов:D.звук.ресурсовСМаяком});

 /* ── ИМЯ ВЕЩИ СОГЛАСУЕТСЯ САМО С СОБОЙ ──
    Имя собиралось из трёх слов, и качество с материалом всегда стояли в
    мужском или среднем роде: игрок слышал «Простое стальной кольцо» и
    «Грубое кожаный кольчуга». Всё произносится вслух, поэтому это слышно
    на каждой находке. Проверяются все сочетания: каждый род вещи × все
    качества × все материалы; у перчаток, сапог и серёг — множественное. */
 const имена=await page.evaluate(()=>{
  const out={плохие:[],всего:0,примеры:[]};
  const окончания={м:{кач:/(ый|ой|ий)$/,мат:/(ый|ой|ий)$/},
   ж:{кач:/ая$/,мат:/(ая|ья)$/},с:{кач:/ое$/,мат:/(ое|ье)$/},мн:{кач:/ые$/,мат:/(ые|ие|ьи)$/}};
  for(const t of ITYPES){
   const род=t[2];
   if(!род||!окончания[род]){out.плохие.push("нет рода: "+t[0]);continue;}
   for(let q=0;q<QUAL.length;q++)for(let r=0;r<MAT.length;r++){
    const n=itemName(t[0],q,r);out.всего++;
    const [кач,мат]=n.split(" ");
    if(!окончания[род].кач.test(кач.toLowerCase())||!окончания[род].мат.test(мат))
     out.плохие.push(n+" ["+род+"]");}
   out.примеры.push(itemName(t[0],1,2));}
  out.качеств=QUAL.length;out.материалов=MAT.length;out.родов=ITYPES.length;
  return out;});
 const QUAL_LEN=имена.качеств,MAT_LEN=имена.материалов;
 check('имя вещи согласовано в роде во всех сочетаниях качества и материала',
  имена.плохие.length===0&&имена.родов>=12&&имена.всего===имена.родов*QUAL_LEN*MAT_LEN,
  {проверено:имена.всего,плохие:имена.плохие.slice(0,6),примеры:имена.примеры});
 check('у каждого рода вещи указан свой род слова',
  !имена.плохие.some(x=>/^нет рода/.test(x)),имена.плохие.filter(x=>/^нет рода/.test(x)));

 /* Голос державы. Пересекая границу, игрок слышит, чья это земля, раньше
    слов — но только если голосов столько же, сколько держав. Список был о
    восьми, а держав стало двенадцать, и последние четыре звучали голосами
    первых четырёх. */
 const голоса=await page.evaluate(()=>({
  держав:EMPIRES.length,голосов:EMPIRE_VOICE.length,
  разных:new Set(EMPIRE_VOICE).size,
  безЗаписи:EMPIRE_VOICE.filter(r=>!(SOUND_BANK[r]&&SOUND_BANK[r].f&&SOUND_BANK[r].f.length)),
  пары:EMPIRES.map((e,i)=>e.short+" → "+EMPIRE_VOICE[i])}));
 check('у каждой державы свой голос, и он в банке',
  голоса.голосов===голоса.держав&&голоса.разных===голоса.держав&&голоса.безЗаписи.length===0,
  голоса);
