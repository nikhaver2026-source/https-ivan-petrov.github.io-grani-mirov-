...
(async()=>{
 ...
 check('первый уровень энциклопедии проходится свайпом',
  разделы.пунктов<=40&&разделы.разделов>=20,{пунктов:разделы.пунктов,разделов:разделы.разделов});
 check('разделы по назначению названы и стоят в каталоге',
  ["Маяки","Бой и оружие","Взаимодействие: вещи, находки и материалы","Ремёсла, добыча и стройка",
   "Люди: голоса, толпа и торг","Музыка мест из фэнтезийных игр","Шаги и поверхности"]
 ...
 check('музыка мира берёт настоящую запись и отпускает её при остановке',
  муз.тема==="mus_town"&&муз.после===null&&муз.бой==="mus_battle",муз);
 ...
 /* Шестнадцать тем SuperTuxKart ушли: игра гоночная, музыку мест дают
    фэнтезийные игры; ушла и флейта — музыки в звуках мира нет. Живых
    записей стихий и ремесла осталось двадцать три. */
 check('двадцать три записи SuperTuxKart на месте и читаются браузером',
  стк.файлов>=23&&стк.битые.length===0,{файлов:стк.файлов,битые:стк.битые.slice(0,5)});
 ...
 const опись=(()=>{
  ...
  /* Список папок здесь раньше перечислялся поимённо, и каждая новая
     свободная игра ломала проверку, пока её папку не впишут. Правило
     же простое: ссылка — это «папка/файл» с звуковым расширением. */
