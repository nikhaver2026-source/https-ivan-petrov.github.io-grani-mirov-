/* ════════════════════════════════════════════════════════════════════════
   ...
   подряд. Теперь правило одно для всего:
     ...
     двойное касание одним пальцем — выполнить названный пункт окна (поле молчит);
     одно касание двумя пальцами   — собрать ресурс (а если открыто окно — закрыть его);
     свайп тремя пальцами вверх    — меню действий;
     свайп тремя пальцами вправо   — сводка «где я» (вниз — инвентарь).
...
(async()=>{
 ...
 // ── 3. Двойное касание одним пальцем по полю не собирает вовсе ──
 await standOnResource();
 await clr();
 await tap();await tap();
 await page.waitForTimeout(300);
 const dbl=await page.evaluate(()=>({запас:{...G.inv},сказано:window.__said.slice()}));
 /* Двойное касание поля — действие здесь (набор 176): на клетке с ресурсом
    это сбор, ровно один раз и с одним ответом — не два, как было, когда
    касание собирало само. */
 check('двойное касание одним пальцем по полю — действие здесь: на ресурсе сбор срабатывает ровно один раз',
  Object.values(dbl.запас).reduce((a,b)=>a+b,0)>=1&&dbl.сказано.filter(t=>/Собрано/.test(t)).length===1,dbl);
 ...
 // ── 6. Окно закрывает свайп двумя пальцами вниз, а не касание ──
 ...
 check('свайп двумя пальцами вниз закрывает открытое окно и ничего при этом не собирает',
  !closed.окно&&Object.keys(closed.запас).length===0,closed);

 // ── 7. Свайп тремя пальцами вправо даёт сводку «где я» ──
 await clr();
 await multiSwipe(3,170,0);
 const status=await said();
 check('свайп тремя пальцами вправо даёт сводку «где я»',
