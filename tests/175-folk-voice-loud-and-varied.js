/* ═══════════════════════════════════════════════════════════════════════
   ...
   1. Громче на деле: запись речи проигрывается и слушается анализатором на
      выходе игры. По новому пути голос не тише −16 дБ и громче прежнего не
      меньше чем на пять децибел, под речью игры — не тише −16 дБ и не меньше
      чем на шесть, и без перегруза: пик ниже нуля.
...
(async()=>{
 ...
 const гр=await page.evaluate(async()=>{
  ...
  const f="voice/prof_torgovec"+VOICE_GEN+".mp3";
  const r={};
  /* Первое проигрывание ещё грузит и разбирает запись и начинает с опозданием —
     его замер врёт на пару децибел. Прогреваем запись и берём среднее двух
     замеров каждого пути. */
  await мер(()=>Spatial.at(f,0,1,{gain:0.95,kind:"npc",maxSec:5}),3.6);
  const два=async fn=>{const a=await мер(fn,3.6),b=await мер(fn,3.6);
   return {rms:Math.round((a.rms+b.rms)*5)/10,peak:Math.max(a.peak,b.peak),кадров:Math.min(a.кадров,b.кадров)};};
  /* Прежний путь — ровно так, как его звали до правки. */
  r.прежде=await два(()=>Spatial.at(f,0,1,{gain:0.95,kind:"npc",maxSec:5}));
  r.теперь=await два(()=>Spatial.at(f,0,1,{gain:VOICE_MIX.gain,kind:"npc",voice:true,maxSec:5}));
  Speech.ducking=true;
  r.преждеПодРечью=await два(()=>Spatial.at(f,0,1,{gain:0.95,kind:"npc",maxSec:5}));
  r.теперьПодРечью=await два(()=>Spatial.at(f,0,1,{gain:VOICE_MIX.gain,kind:"npc",voice:true,maxSec:5}));
 ...
 /* Мера — прежде всего сам уровень голоса: по новому пути он не тише −16 дБ.
    Прибавка к прежнему пути зависит от записи: записи Gemini (3.7) при той же
    громкости −18 LUFS плотнее прежних — пик на два децибела ниже, — поэтому
    по прежнему пути они и так звучат почти на децибел громче, и сжатие
    прибавляет им меньше. Прибавка всё равно не меньше пяти децибел. */
 check('новый путь не тише −16 дБ и громче прежнего не меньше чем на пять децибел',
  гр.теперь.rms>=-16&&гр.теперь.rms-гр.прежде.rms>=5,{прежде:гр.прежде.rms,теперь:гр.теперь.rms});
 /* Под речью игры прежний путь приглушался, а голос — нет: он не тише −16 дБ,
    и прибавка тут больше, чем без речи. */
 check('под речью игры голос не тише −16 дБ и громче прежнего не меньше чем на шесть децибел',
  гр.теперьПодРечью.rms>=-16&&гр.теперьПодРечью.rms-гр.преждеПодРечью.rms>=6,{прежде:гр.преждеПодРечью.rms,теперь:гр.теперьПодРечью.rms});
 ...
 const путь=await page.evaluate(async()=>{
  ...
  const r={};
  const f="voice/say_torg"+VOICE_GEN+".mp3";
