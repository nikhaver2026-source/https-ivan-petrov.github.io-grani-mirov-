...
(async()=>{
 ...
 const данные=await page.evaluate(()=>{
  ...
  r.текст=Traps.text();
  /* Род ловушки узнают ухом раньше слов, а способ поиска отвечает своим
     голосом. Значит, ни один голос не должен повторяться: десять родов и
     семь способов — семнадцать разных записей, и все они настоящие. */
  const голоса=TRAP_KINDS.map(k=>k.звук).concat(TRAP_FINDS.map(w=>w.звук));
  r.голосов=голоса.length;r.разных=new Set(голоса).size;
  r.чужие=голоса.filter(рл=>!/^oc_(trap|find)_/.test(рл));
  r.безФайлов=голоса.filter(рл=>!(SOUND_BANK[рл]&&SOUND_BANK[рл].f&&SOUND_BANK[рл].f.length));
  return r;});
 check('семнадцать своих голосов: у каждого рода и каждого способа своя запись, и ни одна не делится с соседом',
  данные.голосов===17&&данные.разных===17&&данные.чужие.length===0&&данные.безФайлов.length===0,
  {разных:данные.разных,чужие:данные.чужие,безФайлов:данные.безФайлов});
