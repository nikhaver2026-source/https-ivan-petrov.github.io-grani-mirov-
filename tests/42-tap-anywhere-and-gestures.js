...
(async()=>{
 ...
 /* ── 8. Окно закрывает свайп двумя пальцами вниз, а касание — только сбор ── */
 await пальцами(3,-130);
 const былоОткрыто=await page.evaluate(()=>!!activeLayer());
 await пальцами(2,0);
 const всёЕщёОткрыто=await page.evaluate(()=>!!activeLayer());
 await пальцами(2,130);
 check('окно закрывает свайп двумя пальцами вниз, а касание двумя пальцами его не трогает',
  былоОткрыто&&всёЕщёОткрыто&&!(await page.evaluate(()=>!!activeLayer())),
  {былоОткрыто,всёЕщёОткрыто});
