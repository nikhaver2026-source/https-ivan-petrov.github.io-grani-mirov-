...
(async()=>{
 ...
 check('действий тридцать три, у каждого имя, слово и настоящий вызов, а первое — «нет команды»',
  состав.действий===33&&состав.уникД===33&&состав.первое==="none"
 ...
 check('умолчаний пятнадцать, и они в точности прежняя карта жестов: три вниз — инвентарь, двойное тремя — оружие, два вниз — закрыть окно',
  умолчания.сколько===15&&умолчания.всеЖивые
  &&умолчания.инвентарь==="inv"&&умолчания.оружие==="weapon"
  &&умолчания.взаимодействие==="close"&&умолчания.повтор==="repeat"
 ...
 check('раздел настроек показывает все тридцать три фигуры списками из тридцати трёх действий, у каждого списка есть имя, и есть кнопка возврата',
  настройки.ok===true&&настройки.списков===33&&настройки.опций===33
 ...
 const свод=await page.evaluate(()=>{
  ...
  return {есть:!!r,ok:r?r.ok:false,
   ...
   модуль:!!m&&m.shapes.length===33&&m.actions.length===33
