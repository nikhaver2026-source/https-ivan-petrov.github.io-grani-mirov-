...
(async()=>{
 ...
 const пике=await page.evaluate(()=>{
  ...
  const счёт=(место,{ночь=false,навык=null,раз=250}={})=>{
   let п=0,м=0;
   for(let i=0;i<раз;i++){
  ...
  if(мирный)из.мирный=счёт(мирный,{ночь:true,раз:200});
  наЗемлю();
  return из;});
 ...
 check('следопыт бережёт от пике',
  пике.следопыт.п<пике.день.п,{без:пике.день.п,сНавыком:пике.следопыт.п});
