...
(async()=>{
 ...
 check('2. три пальца вверх — список разделов и «Закрыть меню», пунктов разделов не видно; выбор на первом; игра называет число разделов и содержимое выбранного',
  список.открыто&&список.видно.length>=10&&список.видно.slice(0,-1).every(c=>/^amgroup:/.test(c))&&список.видно.slice(-1)[0]==="am:close"
 ...
 check('4. два пальца вниз — снова список, выбор на «Квестах»; второй такой свайп закрывает меню; заново меню открывается списком',
  ...
  &&закрыто&&заново.every(c=>/^amgroup:/.test(c)||c==="am:close"),{назад,закрыто,заново:заново.slice(0,4)});
