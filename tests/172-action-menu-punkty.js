...
(async()=>{
 ...
 check('2. три пальца вверх — список разделов, «Что нового» и «Закрыть меню», пунктов разделов не видно; выбор на первом; игра называет меню и выбранный раздел одним именем',
  ...
  /* С 3.6 раздел называет только своё имя, а меню — себя и выбранное. */
  &&список.курсор===список.видно[0]&&список.подпись.length>2&&!/\d|Двойное|:/.test(список.подпись)
  &&список.сказано==="Меню действий. "+список.подпись,список);
 ...
 check('3. двойное касание по «Квестам и журналу» — в окне только его пункты и «Ко всем разделам меню», выбор на первом, игра назвала раздел',
  ...
  &&квесты.курсор===квесты.своих[0]&&квесты.сказано==="Квесты. Квесты",квесты);
 ...
 check('4. два пальца вниз — снова список, выбор на «Квестах»; второй такой свайп закрывает меню; заново меню открывается списком',
  назад.открыто&&назад.открыт===null&&назад.курсор==="amgroup:quests"&&назад.сказано==="Меню действий. Квесты"
 ...
 const разделы=await page.evaluate(()=>{
  ...
  [...m.querySelectorAll("#amMenu [data-punkt]")].forEach(b=>{
   const id=b.dataset.punkt;const имя=b.dataset.speak;
   ...
   if(в.join()!==ждём.join()||имя!==AM_GROUP_BY_ID[id][0])плохие.push({id,в:в.length,ждём:ждём.length,имя});
   amShowMenu(id);});
  closeActionMenu();
  return {открыто,плохие};});
 check('6. каждый раздел показывает ровно свои уместные пункты и называет себя одним именем',
