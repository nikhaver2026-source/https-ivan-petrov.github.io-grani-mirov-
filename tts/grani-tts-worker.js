...
function нормализовать(t){
 ...
 t=t.replace(/(\d+)[,.](\d+)/g,(m,a,b)=>`${числоСловами(+a)} и ${числоСловами(+b)}`)
  ...
  .replace(/[()[\]{}]/g,",")
  .replace(/[^А-Яа-яЁё\u0301\s.,!?:;-]/g," ")
  .replace(/\s*([.,!?:;])\s*/g,"$1 ").replace(/([.,!?:;])[.,!?:;]+/g,"$1")
  .replace(/\s+/g," ").trim();
 return t;}

/* ── «ю» после согласной ──
   Голос учился на разметке, где за «u» из буквы «ю», стоящей после
   согласной, идёт метка «"» (в карте звуков модели она есть, номер 150).
   Словарь её не хранил, и модель получала не ту запись, на которой училась.
   Мягкая согласная и «u» — это всегда «ю»: после «ч» и «щ» пишут «у», и
   метки там нет. Сверено по всему словарю игры: с правилом разметка
   совпадает с эталонной на всех 24 823 словоформах, без него — на 24 542.
   Без «заглядывания назад» в выражении: его не знает Safari старше 16.4. */
function сЮ(p){return String(p||"").replace(/(tʃ)?ʲ(ˈ?)u(?!")/g,(m,ч,уд)=>ч?m:"ʲ"+уд+'u"');}
/* ── ЗНАК УДАРЕНИЯ ──
   Слово может прийти с ударением, поставленным по смыслу фразы: знак
   U+0301 после ударной гласной («замо́к», «за́мок»). Так размечены фразы, на
   которых учились голоса Gemini (Silero Stress, MIT), так можно поправить
   любое слово в тексте игры. Если словарь ставит ударение туда же — звуки
   из словаря, иначе — по правилам с этим ударением. */
const ЗНАК="\u0301",ГЛ_Б="аеёиоуыэюя",ГЛ_З=new Set("ʌiɑauyoɪeɵɛɔə");
function ударнаяПоЗвукам(p){let k=0;for(const c of p){if(c==="ˈ")return k;if(ГЛ_З.has(c))k++;}return -1;}
function слово(w){
 w=w.toLowerCase();
 if(w.includes("-"))return w.split("-").filter(Boolean).map(слово).join(" ");
 if(w.includes(ЗНАК)){
  const чист=w.split(ЗНАК).join(""),поз=w.indexOf(ЗНАК)-1;
  let номер=-1;for(let i=0,k=0;i<чист.length;i++)if(ГЛ_Б.includes(чист[i])){if(i===поз){номер=k;break;}k++;}
  const есть=словарь&&словарь.get(чист);
  if(есть&&(номер<0||ударнаяПоЗвукам(есть)===номер))return сЮ(есть);
  return сЮ(self.RuG2P?self.RuG2P.g2p(чист,номер<0?undefined:поз):"");}
 if(словарь&&словарь.has(w))return сЮ(словарь.get(w));
 return сЮ(self.RuG2P?self.RuG2P.g2p(w):"");}
...
function звуки(текст){
 ...
 for(const пр of предложения){
  const токены=пр.trim().match(/[А-Яа-яЁё\u0301]+(?:-[А-Яа-яЁё\u0301]+)*|[.,!?:;]/g)||[];
  let s="";
  for(let i=0;i<токены.length;i++){
   const ток=токены[i];
   if(/^[.,!?:;]$/.test(ток)){s=s.replace(/ $/,"")+ток+" ";continue;}
   const н=ток.toLowerCase().split(ЗНАК).join("");
