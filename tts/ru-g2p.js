...
function g2p(слово,ударПоз){
 ...
 for(let i=0;i<w.length;i++){
  ...
  /* Так пишет эталон голоса: мягкий «н» на конце слова и мягкий «л» перед
     согласным звучат твёрдо. */
  const конец=i===w.length-1||(след==="ь"&&i===w.length-2);
  const передСогл=след==="ь"&&w[i+2]&&!ГЛ.includes(w[i+2]);
  if(мягкий&&!((ch==="н"&&конец)||(ch==="л"&&передСогл)))p+="ʲ";
  out.push({p,c:ch});}
